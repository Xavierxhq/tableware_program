...
class ResNetBuilder(nn.Module):
    ...
    def __init__(self, last_stride=1, model_path=None, layers=50, num_of_classes=0):
        super().__init__()
        if layers == 50:
            self.base = ResNet(last_stride, num_of_classes=num_of_classes)
        if layers == 18:
        ...
        if model_path is not None:
            print(model_path.split('/')[-1], 'loaded for resnet building.')

    def forward(self, x):
        global_feat = self.base(x)
        if self.base.num_of_classes == 0:
