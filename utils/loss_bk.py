...
class AugmentedTripletLoss(object):
    ...
    def __call__(self, anchor, positive, negative):
        loss = self.forward(anchor, positive, negative)
        return loss

    def forward(self, anchor, positive, negative):
        return self._triplet_margin_loss(anchor, positive, negative)

    def _triplet_margin_loss(self, anchor, positive, negative):
        # if self.size_average is not None or self.reduce is not None:
        #     reduction_enum = _Reduction.legacy_get_enum(size_average, reduce)
        # else:
        #     reduction_enum = _Reduction.get_enum(reduction)
        return self._triplet_margin_loss_helper(anchor, positive, negative)

    def _triplet_margin_loss_helper(self, anchor, positive, negative):
        # set superparameters
        T1, T2, BETA = -1, .01, .002
        sum_ap, sum_an = .0, .0
        for a, p, n in zip(anchor, positive, negative):
            d_ap = dist(a, p)
            d_an = dist(a, n)
            d_n = max( d_ap-d_an, T1)
            d_p = BETA * max( d_ap, T2 )
            sum_ap += d_p
            sum_an += d_n
        sum_ap /= len(anchor)
        sum_an /= len(anchor)
        return Variable(torch.Tensor([sum_ap+sum_an]), requires_grad=True).cuda()
        # return torch.triplet_margin_loss(anchor, positive, negative, self.margin, self.p, self.eps,
        #                                  self.swap, reduction_enum)
